...
void calculate_s(DG_State* dg ,Err_Steering* Err_Ste )//根据电感状态计算偏差值，并更新偏差值结构体中last量，结果保存在舵机偏差值结构体中
{
    ...
    dg->L_zs_real= (ADC_ReadAverage(ZS, count_dg, RE));
    dg->L_ys_real= (ADC_ReadAverage(YS, count_dg, RE));
...
//使采样最小值为1
    ...
    dg->L_zs_once=(sqrt(((float)dg->L_zs_real)/(dg->L_zs_max)));
    dg->L_ys_once=(sqrt(((float)dg->L_ys_real)/(dg->L_ys_max)));
